...
def predict_disease(image_path):
    start_prep = time.time() #change
    img = Image.open(image_path).resize((256, 256))
    img_array = tf.keras.preprocessing.image.img_to_array(img)
    img_array = tf.expand_dims(img_array, 0)

    end_prep = time.time() #change
    print(f"1. Image Preprocessing: {(end_prep - start_prep) * 1000:.2f} ms") #change

    start_inf = time.time() #change
    predictions = model.predict(img_array)
    score = tf.nn.softmax(predictions[0])
    predicted_class = CLASS_NAMES[np.argmax(score)]
    confidence = (100 * np.max(score))

    end_inf = time.time() #change
    print(f"2. Model Inference: {(end_inf - start_inf) * 1000:.2f} ms") #change
...
def get_agri_innovations(country=None):
    target_location = country if country else "Global"

    prompt = f"""
    Act as an agricultural journalist. 
    1. First, try to find a recent news item, innovation, or outbreak regarding banana cultivation specifically in **{target_location}**.
    2. If there is no significant recent news for {target_location}, provide a major **global** news item about the banana industry instead.

    Format your response with these exact markdown headings:

    ### Headline
    (A very short engaging title in one sentence using 4 or 5 words. don= not add any symbols(ex: -,_,:,',",/, etc). it good for search in google news search bar. If it is {target_location} news, mention the country name in the headline.)
    ...
    try:
        ...
        if headline and summary:
            ...
            return {"headline": f"Banana News ({target_location})", "summary": text}
...
def dashboard():
    ...
    if todays_tasks and weather and 'current' in weather:
        innovations = get_task_recommendation(todays_tasks, weather['current'])
    
    if not innovations:
        innovations = get_agri_innovations(country=current_user.country)
        innovations['is_recommendation'] = False
...
def get_task_recommendation(tasks, weather_data):
    """
    Generates advice based on specific tasks and weather conditions.
    """
    if not tasks or not weather_data:
        return None

    # Extract task descriptions
    task_list = [t['description'] for t in tasks]
    tasks_str = ", ".join(task_list)

    prompt = f"""
    Act as an agricultural expert. 
    The user has the following tasks scheduled for today: "{tasks_str}".
    
    The current weather condition is:
    - Condition: {weather_data['condition']}
    - Temperature: {weather_data['temp']}°C
    - Humidity: {weather_data['humidity']}%
    - Wind: {weather_data['wind_kph']} kph

    Provide a specific recommendation (Headline and Summary) adjusting these tasks for the weather. 
    (e.g., if it is raining and the task is applying fungicide, warn them).
    
    Format your response exactly like this:
    ### Headline
    (Short, punchy advice title)
    ...
    try:
        ...
        if headline and summary:
            return {
                "headline": headline.group(1).strip().replace("**", ""),
                "summary": summary.group(1).strip(),
                "is_recommendation": True # Flag to help frontend identify this
            }
        else:
            return None
    except Exception as e:
        print(f"Gemini API Error (Task Recommendation): {e}")
        return None
...
@app.route('/download_report/<diagnosis_id>')
@login_required
def download_report(diagnosis_id):
    """Generates a PDF report for a specific diagnosis."""
    diagnosis = diagnoses_collection.find_one_or_404({'_id': ObjectId(diagnosis_id)})
    
    if diagnosis['user_id'] != ObjectId(current_user.id):
        user_role = getattr(current_user, 'role', 'user')
        if isinstance(user_role, tuple): user_role = user_role[0]
        if user_role != 'admin':
            return "Unauthorized", 403

    image_absolute_path = os.path.join(app.root_path, 'static', diagnosis['image_path'])
    
 
    html = render_template('results_pdf.html', 
                           diagnosis=diagnosis, 
                           user=current_user,
                           pdf_image_path=image_absolute_path) 
    # Create PDF in memory
    pdf_output = BytesIO()
    pisa_status = pisa.CreatePDF(html, dest=pdf_output)

    if pisa_status.err:
        return f"We had some errors <pre>{html}</pre>"
    ...
    return send_file(pdf_output, 
                     as_attachment=True, 
                     download_name=f"PlantCare_Report_{diagnosis['plant_identifier']}.pdf",
...
def logbook():
    ...
    if filter_disease:
        query['disease_name'] = filter_disease

    # Filter by Status
    if filter_status == 'accurate':
        query['confirmed_accurate'] = True
    elif filter_status == 'inaccurate':
        query['reported_as_inaccurate'] = True
    ...
    if start_date or end_date:
        ...
        if end_date:
            try:
                ...
                date_query['$lt'] = e_date + timedelta(days=1)
            except ValueError: pass
    ...
    user_diagnoses = list(diagnoses_collection.find(query)
                     .sort('timestamp', -1)
                     .skip(skip)
                     .limit(per_page))
...
def delete_diagnosis(diagnosis_id):
    """Deletes a diagnosis and its associated image/tasks."""
...
def contact():
    """Handles contact form submission and saves to Google Sheets."""
    try:
        name = request.form.get('name')
        email = request.form.get('email')
        ...
        # Open the sheet by its EXACT name (case sensitive)
        # Make sure you created a sheet named "PlantCare Messages" in your Google Drive
        sheet = client.open("PlantCare Messages").sheet1 
...
@admin_required
def admin_messages():
    """Reads Google Sheet and displays messages."""
    messages = []
    try:
        # GOOGLE SHEETS CONNECTION
        ...
        # Enumerate to track row index. Start=2 because row 1 is header.
        ...
        flash('Error connecting to Google Sheets.', 'error')

    return render_template('admin_messages.html', messages=messages)

@app.route('/admin/delete_message/<int:row_id>', methods=['POST'])
@login_required
@admin_required
def delete_message(row_id):
    ...
    try:
        ...
        flash('Could not delete message. Please try again.', 'error')
